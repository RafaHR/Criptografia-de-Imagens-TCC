# upload_preprocess_e_matriz_npcr.py
...
def perturbar_pixel_aleatorio(img: np.ndarray) -> np.ndarray:
    ...
    if img.ndim == 3:
        canal = random.randint(0, img.shape[2]-1)
        p[y, x, canal] = (int(p[y, x, canal]) ^ 0x01) % 256
    else:
        p[y, x] = (int(p[y, x]) ^ 0x01) % 256
    return p

def npcr(a: np.ndarray, b: np.ndarray) -> float:
    if shapes_hw(a) != shapes_hw(b):
    ...
    return float(diff.sum()) / float(H * W) * 100.0
...
def main():
    try:
        ...
        n = int(input("Total de repetições do teste: "))
        ...
        for i in range(1, n+1):
            ...
            valor = npcr(normalize_to_uint8(cdf_a), normalize_to_uint8(cdf_b))
            resultados.append(valor)
            print(f"{i} -- NPCR = {valor:.4f}%")

        media = np.mean(resultados)
        minimo = np.min(resultados)
        maximo = np.max(resultados)
        print(f"\nMédia final NPCR em {n} execuções: {media:.4f}%")
        print(f"Valor mínimo: {minimo:.4f}%")
        print(f"Valor máximo: {maximo:.4f}%")

        # --------- PLOT ÚNICO ---------
        plt.figure(figsize=(10,5))
        plt.plot(resultados, marker="o", markersize=3, linestyle="-", color="blue")
        plt.xlabel("Execução")
        plt.ylabel("NPCR (%)")
        plt.title(f"NPCR ao longo de {n} interações com QRNG Simulado\nMédia={media:.4f}% | Min={minimo:.4f}% | Max={maximo:.4f}%")
